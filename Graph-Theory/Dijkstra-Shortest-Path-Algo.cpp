...
vector<pair<ll,ll>>adj[mx];///node->{weight,adj node}
ll vis[mx];
ll dis[mx];///level or distance count.
int n,m;

void dijkstra(int src)
{
    memset(vis,0,sizeof vis);
    for(int i=0;i<=n;i++)dis[i] = infLL; /// Initialization
    dis[src] = 0; /// distance of src is zero

    priority_queue<pair<ll,ll>>pq; /// first check the min one.
    pq.push({0,src});
    ...
    while(!pq.empty())
    {
        ...
        for(auto i : adj[u])
        {
            ...
            ll w = i.second;///distance -> u to b ( Source to
