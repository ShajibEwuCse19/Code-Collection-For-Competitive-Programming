...
int Phi ( int n )
{
    ...
    for ( auto p : prime )
    {
        ...
        if ( n % p == 0 ) /// ϕ ( n ) = ϕ ( p1^a X p2^b X p3^c X ----- ) ==> Create base and power.
        {
            ret = ( ret / p ) ;
...
Theory Part:
=================
Phi ( N ) = ϕ ( N ) = Number of Total Co-Prime or Relatively Prime of N in renge 1 to N. 
...
ϕ ( a x b x c x ----- x N ) = ϕ ( a ) x ϕ ( b ) x ϕ ( c ) x ----------------- ϕ ( N ). So, Phi is a multiplicative function.
    
  
If, N is a Prime Number, ϕ ( N ) = ? 
All numbers are Co-Prime except N because, GCD ( N , N ) = N. but, GCD ( N , 1 to ( N -1 ) ) = 1 where N is Prime.
ϕ ( N ) = N - 1 ( All numbers except N where N is Prime ).
  
  
If, N is a Prime Number, ϕ ( N ^ a ) = ?
Let, N = 2 and a = 3 then, ( 2 ^ 3 ) = 8 and ϕ ( 2 ^ 3 ) = ?.
...
ϕ ( N ^ a )   = N^a - ( N^a / N ) = 8 - ( 8 / base) = 8 - ( 8 / 2 ) = 8 - 4 = 4 ( Total Numbers those are not divisible by N ( base ) where N is a Prime ).
              = N^a ( 1 - 1 / N )
...
ϕ ( P^a x Q^b x R^c )   = ? Where P , Q , R are prime numbers. একটি সংখ্যার প্রাইম ফ্যাক্টোরাইজ হচ্ছে ( P^a x Q^b x R^c ) = N .
ϕ ( P^a x Q^b x R^c )   = ϕ ( P^a ) x  ϕ ( Q^b ) x  ϕ ( R^c )
...
একটি সংখ্যাকে প্রাইম ফ্যাক্টোরাইজেশন করলে বিভিন্ন সংখ্যার পাওয়ার আকারে নির্নয় করতে পারবো যেটি ϕ ( P^a x Q^b x R^c ) এই সূত্র দ্বারা প্রকাশ করতে পারবো।
